<x-layouts.app title="Edit Organisasi" breadcrumb="Master Data / Organisasi / Edit">

    <div class="max-w-[640px]">

        <a href="{{ route('organizations.index') }}" class="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-orange-500 mb-5 no-underline">
        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            <div class="flex items-center gap-3 px-6 py-5 border-b border-slate-100 bg-gradient-to-br from-[#040f2e] to-[#0d2d6b]">
                ...
                <div>
                    <div class="text-white font-semibold text-base">Edit: {{ $organization->name }}</div>
                    <div class="text-blue-300 text-xs mt-px">Perbarui data organisasi</div>
                </div>
            </div>

            <form method="POST" action="{{ route('organizations.update', $organization) }}" class="p-6">
                @csrf @method('PUT')

                @include('organizations._form', ['parents' => $parents, 'organization' => $organization])
                ...
                <div class="flex gap-3 justify-end mt-6 pt-5 border-t border-slate-100">
                    <a href="{{ route('organizations.index') }}" class="px-5 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-600 text-sm font-medium no-underline inline-flex items-center">
                        Batal
                    </a>
