<x-layouts.app title="Laporan Transaksi Departemen">

{{-- Header --}}
<div class="flex items-start justify-between gap-4 mb-5 flex-wrap">
    <div>
        <h2 class="text-lg font-bold text-slate-900 m-0 mb-1">Laporan Transaksi Departemen</h2>
        <p class="text-xs text-slate-400 m-0">
            Rincian pengajuan dana satu departemen, dikelompokkan per bulan
            @if($period)
...
{{-- Filter Bar --}}
<form method="GET" action="{{ route('reports.department-transactions') }}" class="bg-white rounded-xl shadow-sm p-4 mb-4 flex flex-wrap gap-3 items-end">
...
@if(!$department)
<div class="bg-white rounded-xl shadow-sm py-16 px-5 text-center">
    <div class="text-sm font-semibold text-slate-600 mb-1">Belum ada departemen dengan alokasi anggaran</div>
    <div class="text-xs text-slate-400">Departemen perlu punya pagu anggaran pada periode ini agar muncul di sini.</div>
</div>
@else
...
<div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
    <div class="bg-white rounded-xl shadow-sm px-4 py-3.5">
        <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Pagu Departemen</div>
...
<div class="bg-white rounded-xl shadow-sm py-16 px-5 text-center">
    ...
    <div class="text-sm font-semibold text-slate-600 mb-1">Belum ada transaksi</div>
    <div class="text-xs text-slate-400">Departemen ini belum punya pengajuan dana pada periode ini.</div>
</div>
@else
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="text-left text-[11px] text-slate-400 uppercase tracking-wider bg-slate-50/70 border-b border-slate-100">
                    ...
                    <th class="py-2.5 px-4 font-semibold text-right">Realisasi</th>
                </tr>
            </thead>
            <tbody>
                @foreach($months as $month)
                @foreach($month->rows as $fr)
                @php
                ...
                <tr class="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td class="py-2.5 px-3 text-slate-600">{{ $fr->budgetProgram?->name ?? '-' }}</td>
                    ...
                    <td class="py-2.5 px-4 text-right font-mono whitespace-nowrap {{ $fr->realized > 0 ? 'text-green-700' : 'text-slate-300' }}">
                        {{ $fr->realized > 0 ? 'Rp ' . number_format($fr->realized, 0, ',', '.') : '-' }}
                    </td>
                </tr>
                @endforeach
                {{-- Subtotal bulan --}}
                <tr class="bg-green-50/70 border-b border-green-100">
                    <td colspan="5" class="py-2 px-4 text-[11px] font-bold text-green-700 uppercase tracking-wide">Total {{ $month->label }}</td>
                    <td class="py-2 px-3 text-right font-mono font-bold text-slate-800 whitespace-nowrap">Rp {{ number_format($month->diajukan, 0, ',', '.') }}</td>
                    <td class="py-2 px-4 text-right font-mono font-bold text-green-700 whitespace-nowrap">Rp {{ number_format($month->realisasi, 0, ',', '.') }}</td>
                </tr>
                @endforeach
            </tbody>
            <tfoot>
                <tr class="bg-slate-50/70 border-t border-slate-100">
                    <td colspan="5" class="py-2.5 px-4 text-[11px] font-bold text-slate-500 uppercase tracking-widest">Total Keseluruhan</td>
                    ...
                    <td class="py-2.5 px-4 text-right font-mono font-bold text-green-700 whitespace-nowrap">Rp {{ number_format($totals->realisasi, 0, ',', '.') }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
...
<div class="text-[11px] text-slate-400 mt-3">
    Realisasi = dana dicairkan − pengembalian dana terkonfirmasi.
</div>
@endif
@endif
@endif
