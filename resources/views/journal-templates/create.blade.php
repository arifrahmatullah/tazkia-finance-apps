<x-layouts.app title="Buat Template Jurnal">
...
<h1 class="text-xl font-bold text-slate-900 m-0 mb-0.5">Buat Template Jurnal</h1>
<p class="text-sm text-slate-400 mb-5">Susun pola akun debit–kredit untuk transaksi yang sering berulang</p>
...
<form method="POST" action="{{ route('journal-templates.store') }}" id="jt-form">
@csrf
...
<div class="bg-white rounded-xl shadow-sm p-6 mb-3.5">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div class="flex flex-col gap-1.5">
            ...
            <input type="text" name="code" value="{{ old('code') }}" maxlength="50" placeholder="Contoh: TJ-001"
        ...
        <div class="flex flex-col gap-1.5">
            ...
            <input type="text" name="name" value="{{ old('name') }}" maxlength="255" placeholder="Contoh: Pembayaran Gaji Karyawan"
        ...
        <div class="flex flex-col gap-1.5">
            ...
            <input type="text" name="category" value="{{ old('category') }}" maxlength="100" list="category-list" placeholder="Contoh: Operasional, Penggajian (opsional)"
        ...
        <div class="flex flex-col gap-1.5">
            ...
            <input type="text" name="tags" value="{{ old('tags') }}" maxlength="255" list="tag-list" placeholder="Contoh: SPMB, SIAKAD (pisahkan koma, opsional)"
                class="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm text-slate-800 bg-white outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition-colors">
...
<div class="flex gap-3 justify-end">
    ...
    <button type="submit" class="px-6 py-2.5 rounded-xl text-sm font-semibold bg-gradient-to-br from-orange-400 to-orange-500 text-white border-0 cursor-pointer hover:-translate-y-px transition-all">Simpan Template</button>
</div>
</form>
...
async function loadAccounts(orgId) {
    if (!orgId) { accountOptions = []; rebuildAllAccountSelects(); return; }
    try {
...
function rebuildAllAccountSelects() {
    document.querySelectorAll('#details-body select[name*="[account_id]"]').forEach(sel => {
        const cur = sel.value;
...
document.addEventListener('DOMContentLoaded', function() {
    if (oldDetails && Object.keys(oldDetails).length > 0) {
        Object.values(oldDetails).forEach(d => addDetailRow(d));
    } else {
