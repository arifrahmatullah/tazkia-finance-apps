        ...
        <form method="POST" action="{{ route('budget-programs.update', $budgetProgram) }}" class="px-5 py-5 flex flex-col gap-4">
            ...
            <div>
                ...
                <select name="type" id="type-select" onchange="showTypeInfo(this.value)"
                    class="w-full px-3.5 py-2.5 border {{ $errors->has('type') ? 'border-red-400' : 'border-slate-200' }} rounded-xl text-sm text-slate-700 bg-white outline-none focus:border-orange-400 transition-colors">
                    <option value="">— Pilih jenis program —</option>
                    @foreach(\App\Models\BudgetProgram::TYPES as $val => $label)
                    <option value="{{ $val }}" {{ old('type', $budgetProgram->type) === $val ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div>
                <label class="block text-xs font-semibold text-slate-600 mb-1.5">
                    ...
                    <span class="font-normal text-slate-400 ml-1">berapa kali bayar dalam periode (maks. 24×)</span>
                </label>
