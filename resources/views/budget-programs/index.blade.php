...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-5">
    ...
    <div class="bg-white rounded-xl border border-slate-100 shadow-sm px-4 py-3.5">
        ...
        <div class="text-[10px] text-slate-400 uppercase tracking-wide">Pagu (Keuangan)</div>
        <div class="text-sm font-bold font-mono text-slate-800 mb-2">Rp {{ number_format($s['pagu'], 0, ',', '.') }}</div>
        <div class="flex items-end justify-between gap-3 mb-2">
            <div>
                <div class="text-[10px] text-slate-400 uppercase tracking-wide">Total Program</div>
                <div class="text-sm font-bold font-mono {{ $over ? 'text-red-600' : 'text-orange-600' }}">
                    Rp {{ number_format($s['terpakai'], 0, ',', '.') }}
                </div>
            </div>
            <div class="text-right">
                <div class="text-[10px] text-slate-400 uppercase tracking-wide">Sisa Pagu</div>
        ...
        <div class="flex items-end justify-between gap-3 pt-2 border-t border-slate-100">
            <div>
                <div class="text-[10px] text-slate-400 uppercase tracking-wide">Sudah Dicairkan</div>
                <div class="text-sm font-bold font-mono text-blue-600">
                    Rp {{ number_format($s['cair'], 0, ',', '.') }}
                </div>
            </div>
            <div class="text-right">
                <div class="text-[10px] text-slate-400 uppercase tracking-wide">Sisa Uang Dari Program</div>
        ...
        <a href="{{ route('budget-programs.create', ['department_id' => $s['department_id']]) }}"
            class="mt-3 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-orange-50 text-orange-600 text-xs font-semibold hover:bg-orange-100 transition-colors no-underline">
            <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>
            Buat Program Kerja (dibantu Superadmin)
        </a>
        @endif
    </div>
    @endforeach
