...
<div class="bg-white rounded-xl shadow-sm overflow-hidden">
    @if($programs->isEmpty())
        <div class="py-12 text-center text-slate-400">
            ...
            <p class="text-xs text-slate-300 mt-1">Program kerja ditambahkan melalui halaman Pagu Anggaran.</p>
        </div>
    @else
    <table class="w-full border-collapse">
        <thead>
            <tr class="bg-slate-50 border-b border-slate-100">
                ...
                <th class="px-5 py-3 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wide">Akun COA</th>
        ...
        <tbody>
            ...
            <tr class="border-b border-slate-50 hover:bg-slate-50/60 transition-colors last:border-0">
                ...
                <td class="px-5 py-3.5 align-middle">
                    @if($prog->account)
                        <div class="text-xs font-mono text-slate-500">{{ $prog->account->code }}</div>
                        <div class="text-xs text-slate-600">{{ $prog->account->name }}</div>
                    @else
